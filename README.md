...
* Support for .NET Framework 2.0, 3.5, 4.0 and 4.5.2 was dropped. Use version 1.6 if you need those.
* The public Doctor Dump web service proxy types generated by the old Visual Studio "Web Reference" (`HttpsCrashReporterReportUploader`, `HttpCrashReporterReportUploader`, `SendAnonymousReportCompletedEventArgs`, `SendAdditionalDataCompletedEventArgs` and their event handler delegates) were removed. The data contract types in `CrashReporterDotNET.com.drdump` (`Response`, `ErrorResponse`, etc.) are unchanged.
